 ...
 * Student ID: 12199888
 */
public class LoginPageController implements Initializable, LoginPresenter.LoginModelObservable {
    ...
    public void onLoginSuccess(ActionEvent event, User user) {
        ...
        try {
            if(user.getRole() == 0) {
                ...
                Parent root = fxmlLoader.load();
                AdminPageController admin = fxmlLoader.<AdminPageController>getController();
                admin.user = user;
                Scene scene = new Scene(root);
                stage.setScene(scene);
            }
